...
void Grid_Manager::initial()
{
	...
	// compuate reference morton code xmorton_ref, ymorton_ref and zmorton_ref
...
D_uint niz = static_cast <D_uint> ((C_domain[5]-C_domain[2]) / C_dx + C_eps);
	...
	Morton_Assist::mortony_max = static_cast <D_morton> (Morton_Assist::pointer_me->morton_encode(0, niy)) << Morton_Assist::bit_otherlevel;
#endif
#if (C_DIMS == 3)
	Morton_Assist::mortonx_max = static_cast <D_morton> (Morton_Assist::pointer_me->morton_encode(nix, 0, 0)) << Morton_Assist::bit_otherlevel;
	Morton_Assist::mortony_max = static_cast <D_morton> (Morton_Assist::pointer_me->morton_encode(0, niy, 0)) << Morton_Assist::bit_otherlevel;
	Morton_Assist::mortonz_max = static_cast <D_morton> (Morton_Assist::pointer_me->morton_encode(0, 0, niz)) << Morton_Assist::bit_otherlevel;
#endif

	// inner: the finest mesh near the geometry
	// std::cout << "Before generate_inner" << std::endl;
	generate_inner();
	// std::cout << "After generate_inner" << std::endl;
...
// #endif
		for (D_uint ipoint = 0; ipoint < Solid_Manager::pointer_me->shape_solids.at(ishape).numb_nodes; ++ipoint)
		{
			...
			//          o-----o
			//         /     /|
			//        o-----o |                If a solid point appears inside a cube(lattice) with space of inner_dx,
			//        | o-*-|-o-----o            the eight points connected to (xint,yint,zint) are signed as the "near_solid".
			//        |/    |/     /|          * : solid point
			//        o-----@-----o |          o : node
			//        |  *  |  *  | |          @ : map_node_near_solid[MortonKey].icount = 3
			//        |     |     | /
			//        o-----o-----o
			//        ^
			//  (xint,yint,zint)
		...
		// std::cout << "After [find_nodes_near_solid]: " << refine_nodes.size() << std::endl;
		//
		// std::ofstream refine_nodes_file;
		// refine_nodes_file.open("refine_nodes_1.vtk", std::ios::out);
		// refine_nodes_file << "# vtk DataFile Version 2.0\n\n";
		// refine_nodes_file << "ASCII\nDATASET POLYDATA\n";
		// refine_nodes_file << "POINTS " << refine_nodes.size() << " float\n";
		// for (auto refine_nodes_iter : refine_nodes) {
		// 	D_real refine_point_x, refine_point_y, refine_point_z;
		// 	Morton_Assist::compute_coordinate(refine_nodes_iter.first, ilevel, refine_point_x, refine_point_y, refine_point_z);
		// 	refine_nodes_file << refine_point_x << " " << refine_point_y << " " << refine_point_z << "\n";
		// }
		// refine_nodes_file << "VERTICES " << refine_nodes.size() << " " << 2*refine_nodes.size() << "\n";
		// for (int i = 1; i <= refine_nodes.size(); ++i) {
		// 	refine_nodes_file << "1 " << i << "\n";
		// }
		// refine_nodes_file.close();

		D_morton morton_xyz = Morton_Assist::morton_xyz.at(ilevel);
		D_morton morton_xyz_1 = morton_xyz;
		morton_xyz_1.flip(); // All background bits and all bits of levels != ilevel are 1, and bits at ilevel zyx=000.
		D_morton morton_temp;

		for (D_map_Infor_near_solid::iterator iter = map_node_near_solid.begin(); iter != map_node_near_solid.end(); ++iter)
		{
			morton_temp = iter->first & morton_xyz_1; // the MortonCode of the nodes_near_solid in the (C_max_level-1) level
			if (nearest_center.find(morton_temp) == nearest_center.end())
			{
				nearest_center.insert({ morton_temp, flag_refine }); // nearest_center is the center node at the (C_max_level-1) level around the finest node
			}
		}
	...
	// output the refine_nodes as point clouds
	//
	// std::ofstream refine_nodes_file;
	// char file_name[1024];
	// sprintf(file_name, "refine_nodes_%d.vtk", ilevel);
	// refine_nodes_file.open(file_name, std::ios::out);
	// refine_nodes_file << "# vtk DataFile Version 2.0\n\n";
	// refine_nodes_file << "ASCII\nDATASET POLYDATA\n";
	// refine_nodes_file << "POINTS " << refine_nodes.size() << " float\n";
	// for (auto refine_nodes_iter : refine_nodes) {
	...
	for (unsigned int ishape = 0; ishape < Solid_Manager::pointer_me->numb_solids; ++ishape)
	{
		if (Solid_Manager::pointer_me->shape_solids.at(ishape).bool_enclosed == true)
		{
			...
			std::cout << "Solid_Manager::pointer_me->shape_solids.at(ishape).x,y,z = " 
					  << "(" << Solid_Manager::pointer_me->shape_solids.at(ishape).x0 << " , "
					  << Solid_Manager::pointer_me->shape_solids.at(ishape).y0 << " , "
					  << Solid_Manager::pointer_me->shape_solids.at(ishape).z0 << ")" << std::endl;
			...
			D_morton keyin = 0;  // the point inside the solid
...
#endif
			// search in -x direction from the solid center until meet the first point in refine_node
			...
			// search in -y direction from the solid center until meet the first point in refine_node
			if (bool_key_Nfound)
			{
				morton_temp = morton_center;
				icount = 0;
	...
	{
		D_morton outest_refine_node;
		D_real max_distance_refineNode2solidPoint = 0.;
		Solid_Node solid_point = ishape.node.at(0);
		for (auto i_refine_nodes : refine_nodes) {
			D_vec refine_node_xyz;
			Morton_Assist::compute_coordinate(i_refine_nodes.first, ilevel, refine_node_xyz.x, refine_node_xyz.y, refine_node_xyz.z);
			D_real l = Shape::two_points_length(D_vec(solid_point.x, solid_point.y, solid_point.z), refine_node_xyz);
			if (l > max_distance_refineNode2solidPoint) {
				outest_refine_node = i_refine_nodes.first;
				max_distance_refineNode2solidPoint = l;
			}
		}

		// flood fill from outest_refineNode
		// mark fluid nodes in the refine_nodes
		//   and delete nodes inside the solid
	...
	for (int iboundary = 1; iboundary < C_overlap * 2 + 1; ++iboundary)
	{
		for (D_mapint::iterator iter = gr_inner.fine2coarse.at(iboundary).begin(); iter != gr_inner.fine2coarse.at(iboundary).end(); ++iter)
		{
			if (refine_nodes.find(iter->first) == refine_nodes.end())
			{
				refine_nodes.insert(make_pair(iter->first, std::array<int, 2>{flag_interface[iboundary], 0}));
			}
			else
			{
				refine_nodes[iter->first].at(0) = flag_interface[iboundary];
			}
		}
	}
...
void Grid_Manager::find_nodes_near_solid(unsigned int ilevel, std::array<D_real, C_DIMS> xyz, D_mapint2 &refine_nodes)
{
	...
	#if (C_DIMS==3)
	z_index = xyz.at(2) / grid_ptr->dx;
	// zint = static_cast<D_uint>((xyz.at(2)-C_domain[2]) / grid_ptr->dx + C_eps);
...
#endif
	D_real x, y, z; // (x,y,z) is in the coordinate of the center(xb_domain/2, yb_domain/2, zb_domain/2) new system
	...
	// Judge the finest nodes close to the solid point whether has been added into the set of "refine_nodes"
	//   if not, add the absent finest node into "refine_nodes"
...
void Grid_Manager::mark_ghost_points(unsigned int ilevel, D_mapint2& refine_nodes)
{
	...
	for (D_map_Infor_near_solid::iterator iter = map_node_near_solid.begin(); iter != map_node_near_solid.end(); ++iter)
	{
		...
		extend_temp_z0 = extend_z1; extend_temp_z1 = extend_z0;
#endif
		// Limit the extended ghost nodes inside the solid to prevent exceeding the entire geometry
...
#endif
#endif
		// search nodes in the searching area
#if (C_DIMS == 2)
		morton_temp = Morton_Assist::pointer_me->morton_encode(static_cast<D_int>(x / gr_inner.dx + C_eps) - extend_temp_x0, static_cast<D_int>(y / gr_inner.dx + C_eps) - extend_temp_y0);
		morton_temp1 = morton_temp;
#endif
...
* @param[out] nodes_level1        nodes (C_max_level - 1) within the predefined distance (C_extend_inner/2) from solid nodes.
* @param[out] refine_nodes        nodes (C_max_level) near the solid.
* @note Here param[in]:ilevel is C_max_level.
*/
...
void Grid_Manager::search_nodes_near_solid(const unsigned int ilevel, D_mapint &nearest_center, D_mapint &nodes_level1, D_mapint2 &refine_nodes)
{
	...
	uint extend_x0 = C_extend_inner/2; // extend in ilevel-1
	uint extend_x1 = C_extend_inner/2; // extend in ilevel-1
	uint extend_y0 = C_extend_inner/2; // extend in ilevel-1
	uint extend_y1 = C_extend_inner/2; // extend in ilevel-1
	uint extend_z0 = C_extend_inner/2; // extend in ilevel-1
	uint extend_z1 = C_extend_inner/2; // extend in ilevel-1
	...
	for (D_mapint::iterator iter = nearest_center.begin(); iter != nearest_center.end(); ++iter)
	{
		...
		if ((x - extend_x0 * gr_inner.dx) < C_domain[0]) {
			extend_temp_x0 = static_cast<unsigned int>((x - C_domain[0]) / gr_inner.dx + C_eps); // extend extend_x0 from the nearest_center
		}
		if ((x + extend_x1 * gr_inner.dx) > C_domain[3]) {
			extend_temp_x1 = static_cast<unsigned int>((C_domain[3] - x) / gr_inner.dx + C_eps);
		}
		if ((y - extend_y0 * gr_inner.dx) < C_domain[1]) {
			extend_temp_y0 = static_cast<unsigned int>((y - C_domain[1]) / gr_inner.dx + C_eps);
		}
		if ((y + extend_y1 * gr_inner.dx) > C_domain[4]) {
			extend_temp_y1 = static_cast<unsigned int>((C_domain[4] - y) / gr_inner.dx + C_eps);
		}
		if ((z - extend_z0 * gr_inner.dx) < C_domain[2]) {
			extend_temp_z0 = static_cast<unsigned int>((z - C_domain[2]) / gr_inner.dx + C_eps);
		}
		if ((z + extend_z1 * gr_inner.dx) > C_domain[5]) {
			extend_temp_z1 = static_cast<unsigned int>((C_domain[5] - z) / gr_inner.dx + C_eps);
		}
#endif
#if (C_DIMS == 2)
		morton_temp = Morton_Assist::pointer_me->morton_encode(static_cast<D_int>(x / gr_inner.dx + C_eps) - extend_temp_x0 * 2, static_cast<D_int>(y / gr_inner.dx + C_eps) - extend_temp_y0 * 2);
	...
	for (D_mapint::iterator iter = nodes_level1.begin(); iter != nodes_level1.end(); ++iter)
	{
		for (D_int i_q = 0; i_q < C_Q; ++i_q) {
			D_morton node_inside_nodes_level1 = Morton_Assist::find_neighbor(iter->first, C_max_level, ex[i_q], ey[i_q], ez[i_q]);
			refine_nodes.insert(make_pair(node_inside_nodes_level1, std::array<int, 2> { flag_refine, 0 }));
		}
/*
		if (refine_nodes.find(morton_temp_center) == refine_nodes.end())
		{
...
* @param[in]  exist_nodes              existed nodes.
			...
			if (x_negative)
			{
				...
				if (exist_nodes.find(morton_temp0) == exist_nodes.end())
				{
					...
					if (ilevel == C_max_level)
					{
						...
						{
							grid_ptr_coarse->coarse2fine.at(iboundary0_coarse).insert({ iter->first, flag_interface[0] });
						}
					}
...
*/
template<typename  T> 
void Grid_Manager::search_inter_boundary(unsigned int ilevel, unsigned int iboundary1, unsigned int iboundary2, unsigned int iboundary2_coarse, D_mapint map_outer, T refine_nodes, std::array<D_mapint, 2> &boundary_x_temp, std::array<D_mapint, 2> &boundary_y_temp, std::array<D_mapint, 2> &boundary_z_temp)
{
	// std::string code_string = "0000000000000000000000000000000000001010101000000001101011110110";
	// D_morton codex(code_string);
...
* @param[in]  ilevel    refinement level.
* @note       Mesh generation is based on the numerical boundary (coarse2fine) obained in the previous step (ilevel + 1). The mesh is expanded one layer by one layer.
*/
void Grid_Manager::generate_intermediate(unsigned int ilevel)
{
	...
	for (D_mapint::iterator iter = map_boundary_temp.begin(); iter != map_boundary_temp.end(); ++iter)
	{
		if ((iter->first&morton_xyz) == 0)
		{
			...
			if (!bool_N_boundary)
			{
				grid_ptr_coarse->coarse2fine.at(0).insert({ iter->first, flag_interface[0] });
			}
		}
	}
	...
	// set flag at numerical boundaries as flag_interface
	for (int iboundary = 1; iboundary < C_overlap * 2 + 1; ++iboundary)
	{
		for (D_mapint::iterator iter = grid_ptr->fine2coarse.at(iboundary).begin(); iter != grid_ptr->fine2coarse.at(iboundary).end(); ++iter)
		{
			gr_NoIB.at(ilevel).grid[iter->first].flag = flag_interface[iboundary];
		}
...
void Grid_Manager::generate_background()
{
	...
	unsigned int cnt = 0; ///> calculate the digits of the background Morton code
	while (n_max_temp >>= 1) cnt++; // until n_max_temp >> ->0 (log_2)
...
//#endif
	...
	for (D_mapint::iterator iter = grid_ptr->coarse2fine.at(iboundary0_coarse).begin(); iter != grid_ptr->coarse2fine.at(iboundary0_coarse).end(); ++iter)
	{
		gr_NoIB.at(ilevel).grid[iter->first].flag = flag_interface[0];
	}
...
void Grid_Manager::flood_fill_background(unsigned int ilevel, D_morton &key_in)
{
	...
	while (!stk.empty())
	{
		...
		stk.pop_back();
		morton_temp = seed;
		if ((gr_NoIB.at(ilevel).grid.find(morton_temp) == gr_NoIB.at(ilevel).grid.end()) || (gr_NoIB.at(ilevel).grid[morton_temp].flag == flag_interface[0]))
		{
			continue;
		}
		else
		{
			gr_NoIB.at(ilevel).grid.erase(morton_temp); // delete the refined node existed in background level0
...
#endif
			morton_temp0 = Morton_Assist::find_y1(morton_temp0, ilevel); // traverse the boundary
			morton_temp1 = Morton_Assist::find_y1(morton_temp1, ilevel);
		}
#if(C_DIMS == 3)
...
void Grid_Manager::delete_refineNodes_inside_solid(const uint ilevel, const D_morton floor_fill_point, D_mapint2 &refine_nodes)
{
	...
	while (!bfs_queue.empty()) {
		...
		for (D_int i_q = 0; i_q < C_Q-1; ++i_q) {
			D_morton ngbr_ = ngbr_code[i_q];
			auto ngbr_ptr = refine_nodes.find(ngbr_);
	...
	// copy rst_refine_nodes to refine_nodes
	refine_nodes.clear();
	for (auto &it : rst_refine_nodes) {
